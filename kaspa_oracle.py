...
function drawBubbles(trades) {
    ...
    trades.forEach((t, i) => {
        ...
        if (t.whale) {
            ctx.shadowBlur = 0;
            ctx.font = Math.round(r*0.8)+'px Arial';
...
async function updateData() {
    try {
        ...
        const appleEl = document.getElementById('price-apple');
        priceEl.innerText = 'KAS $' + Number(data.last_price).toFixed(5);
        ...
        if (data.prediction) {
            ...
            const color = p.direction === 'UP' ? '#22c55e' : p.direction === 'DOWN' ? '#ef4444' : '#f59e0b';
            document.getElementById('pred-direction').innerText = arrow + ' ' + p.direction;
            document.getElementById('pred-direction').style.color = color;
            document.getElementById('pred-pct').innerText = (p.change_pct >= 0 ? '+' : '') + p.change_pct.toFixed(2) + '%';
            document.getElementById('pred-conf').innerText = 'conf: ' + p.confidence + '%';
            updateMeter(p.change_pct);
            updateDolphin(p.direction, data.whale);
        }

        if (data.accuracy) {
            document.getElementById('acc-pct').innerText = data.accuracy.pct.toFixed(1) + '%';
            document.getElementById('acc-stats').innerText = '(' + data.accuracy.correct + '/' + data.accuracy.total + ')';
        }

        if (data.whale && data.whale.ts !== lastWhalTs) {
            lastWhalTs = data.whale.ts;
            showWhale(data.whale.amount);
        }

        document.getElementById('s-time').innerText = new Date().toLocaleTimeString();
        document.getElementById('s-mom').innerText = (data.momentum >= 0 ? '+' : '') + data.momentum.toFixed(2) + '%';

        draw(data.trades, data.last_price, data.prediction);
...
def flask_data():
    try:
        ...
        for snap in history:
            trades.append({
                "price": snap["price"],
                "side": "BUY" if snap.get("whale_kas", 0) > 0 else "SELL",
                "value": snap.get("whale_kas", 0) * snap["price"],
                "whale": snap.get("whale_kas", 0) >= KAS_WHALE_THRESHOLD
            })

        kas_price = history[-1]["price"] if history else KAS_USD_ESTIMATE
        momentum = _calc_momentum()

        pred = None
        if history:
            last = history[-1]
            if last.get("whale_kas", 0) > 0:
                tier = "MEGA" if last["whale_kas"] >= MEGA_WHALE else "BIG" if last["whale_kas"] >= BIG_WHALE else "WHALE"
                pd = predict_5min_price(kas_price, last["whale_kas"], last["whale_count"], tier)
            else:
                pd = predict_no_whale(kas_price)
            pred = {
                "direction": pd["direction"],
        ...
        if history:
            last = history[-1]
            if last.get("whale_kas", 0) >= KAS_WHALE_THRESHOLD:
                whale = {"amount": last["whale_kas"], "ts": int(last["ts"] * 1000)}

        return json.dumps({
            "trades": trades[-100:],
            "last_price": kas_price,
            "prediction": pred,
            "whale": whale,
